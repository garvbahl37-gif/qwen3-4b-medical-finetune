...
| install | installs `transformers==5.5.0`, then `peft==0.19.1` with `--no-deps`, then `datasets` | |
| 2. Verify the install | prints the torch, transformers and peft versions | transformers is not 5.5.0 or peft is not 0.19.1. The message says the install did not take effect on this kernel image: restart the session, then Run All again |
