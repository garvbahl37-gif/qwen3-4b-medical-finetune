...
| Loading the adapter fails with `ImportError: Found an incompatible version of torchao`. | The image ships torchao 0.10.0, and peft 0.19.1 raises on any torchao older than 0.16.0 while it wraps each layer. This stopped the first evaluation run. | The install cell uninstalls torchao, and cell 2 wraps one tiny layer with LoRA, so a package like this stops the run in seconds. |
